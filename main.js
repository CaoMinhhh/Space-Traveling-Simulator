...
const rockTexture = textureLoader.load('https://upload.wikimedia.org/wikipedia/commons/2/2d/Karahayit_1998_01.jpg');
// rockNormal tuy load về nhưng chúng ta tạm không dùng để tránh lỗi bóng đen (như đã bàn)
const rockNormal = textureLoader.load('https://upload.wikimedia.org/wikipedia/commons/2/2c/Generic_Celestia_asteroid_texture.jpg'); 
...
renderer.shadowMap.type = THREE.PCFSoftShadowMap; // Bóng mềm
...
// --- 6. ASTEROIDS (OPTIMIZED MODEL) ---
const asteroids = [];
const asteroidCount = 6; 

// [FIX] Dùng DodecahedronGeometry detail = 0 (Chuẩn Low-poly)
const asteroidGeo = new THREE.DodecahedronGeometry(2.5, 0); 

const asteroidMat = new THREE.MeshStandardMaterial({
    map: rockTexture, 
    normalMap: null, // [FIX] Tắt NormalMap để tránh lỗi bóng đen
    displacementMap: null, 
    color: 0x999999, // Màu xám đá sáng
    roughness: 0.8,  
    metalness: 0.0   
});
...
function createAsteroid(initialZ) {
    ...
    // Random scale để tạo sự đa dạng
    asteroid.scale.set(
        1 + Math.random() * 0.5, 
        1 + Math.random() * 0.5, 
        1 + Math.random() * 0.5
...
// [FIX] HÀM SINH VỊ TRÍ THEO VÀNH ĐAI (SAFE RING)
function resetAsteroidPosition(asteroid, zPos = -500) {
    asteroid.position.z = zPos;
    
    const angle = Math.random() * Math.PI * 2;
    const radius = 9 + Math.random() * 30; // Bán kính vành đai từ 9 đến 39
    
    asteroid.position.x = Math.cos(angle) * radius;
    asteroid.position.y = Math.sin(angle) * radius - 1.5;
...
// [FIX] Tắt castShadow để loại bỏ hoàn toàn lỗi răng cưa/đen bóng (Shadow Acne)
